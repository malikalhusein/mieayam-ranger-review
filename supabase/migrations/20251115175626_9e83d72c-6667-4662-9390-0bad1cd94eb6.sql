-- Update overall_score formula to use 85% of price and 8-minute tolerance
...
ADD COLUMN overall_score NUMERIC GENERATED ALWAYS AS (
  CASE 
    WHEN (price * 0.85 + (COALESCE(service_durasi, 8) - 8) * 100) <= 0 THEN 0
    ELSE (
      (
      ...
      (price * 0.85 + (COALESCE(service_durasi, 8) - 8) * 100)::numeric
    ) * 1000
  END
) STORED;